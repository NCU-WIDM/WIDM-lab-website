...
const MemberPage = () => {
  ...
  const headers = [
    ...
    },
    {
      ...
      isShow: 'true',
      type: 'Date',
      dateType: ['month', 'YYYY-MM'] as [PickerMode, string],
      style: {
        minWidth: '150px',
        whiteSpace: 'normal',
        wordBreak: 'break-word',
    ...
    try {
      ...
      if (editData) {
        response = await defaultHttp.patch(
          `${processDataRoutes.member}/${editData.id}`,
          newMember,
          { headers: storedHeaders() },
        );
      } else {
        response = await defaultHttp.post(processDataRoutes.member, newMember, {
          headers: storedHeaders(),
        });
      }
      setIsAdding(false);
  ...
  const deleteMember = async (id: number) => {
    try {
      ...
      await defaultHttp.delete(`${processDataRoutes.member}/${id}`, {
        headers: storedHeaders(),
      });
